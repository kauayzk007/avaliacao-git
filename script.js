...
// Adiciona um "ouvinte de eventos" que dispara uma função quando o mouse passa por cima do botão
...
// Função que cria várias mensagens de uma só vez
function adicionarVariasMensagens(qtd) {
    // Usa um loop for para repetir a ação várias vezes
    for (let i = 1; i <= qtd; i++) {
        messageCount++;

        const newParagraph = document.createElement('p');
        newParagraph.classList.add('new-text');
        newParagraph.textContent = `Mensagem em loop #${messageCount}`;
        textContainer.appendChild(newParagraph);
    }
}

// Exemplo de uso: adiciona 5 mensagens automáticas quando a página carrega
window.addEventListener('load', function() {
    adicionarVariasMensagens(5);
});
